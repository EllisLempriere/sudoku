...
namespace Sudoku.Library.Tests.Solver
{
    ...
    {
        [Test]
        public void TryApply_OneHiddenSingle_Resolved()
        {
            // arrange
            var grid = CreateGridWithHiddenAndNakedSingles();
            ...
            Assert.AreEqual(5, grid.Cells[0, 5].Number.Value);
            Assert.IsTrue(grid.AllRegionsAreValid);
        }

        private Grid CreateGridWithHiddenAndNakedSingles()
        {
            var grid = CreateGrid(_veryEasy002SolvedData);

            grid.Cells[0, 5].Number = null;
            grid.Cells[0, 5].SetCandidates(new[] { 3, 5 });

            grid.Cells[0, 8].Number = null;
            grid.Cells[0, 8].SetCandidate(3);

            grid.Cells[2, 5].Number = null;
            grid.Cells[2, 5].SetCandidate(3);
